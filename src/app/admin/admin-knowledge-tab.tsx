  ...
  return (
    <div className="rounded-lg" style={{ border: "1px solid var(--color-border)", background: "var(--color-surface-overlay)" }}>
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        <button type="button" onClick={onRemove} disabled={removing} title={t("Borrar fuente")} className="inline-flex h-7 w-7 items-center justify-center rounded-md disabled:opacity-40" style={{ color: "var(--color-error)" }}>
      ...
      {open && (
        <div className="space-y-1.5 border-t px-3 py-2" style={{ borderColor: "var(--color-border)" }}>
          {chunksQ.isLoading ? (
            ...
            <p className="py-2 text-center text-xs" style={{ color: "var(--color-subtle)" }}>{t("Sin fragmentos.")}</p>
          ) : (
            (chunksQ.data ?? []).map((c) => (
...
export function AdminKnowledgeTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title={t("Cargar conocimiento")} desc="Pegá tu estrategia, FAQ o docs. Se parte en fragmentos, se embebe y el asistente lo usa para responder con tu criterio.">
        <div className="mb-2 flex flex-wrap items-center gap-1.5">
          <PencilLine className="h-3.5 w-3.5" style={{ color: "var(--color-subtle)" }} />
          <span className="mr-1 text-[11px]" style={{ color: "var(--color-subtle)" }}>{t("Fuente:")}</span>
          ...
          <input value={source} onChange={(e) => setSource(e.target.value)} placeholder={t("o escribí una…")} className="w-32 rounded-md px-2.5 py-1 text-[11px] outline-none" style={{ background: "var(--color-surface-overlay)", border: "1px solid var(--color-border)", color: "var(--color-foreground)" }} />
        </div>
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={6} placeholder={t("Pegá acá tu conocimiento…")}
      ...
      <Card title={t("Reindexar desde la plataforma")} desc="Trae al asistente lo que ya vive en tu DB: la KB de ángulos y el catálogo de apps.">
      ...
      {/* Contenido cargado */}
      <Card title={t("Contenido cargado")} count={total}>
        {sourcesQ.isLoading ? (
          <div className="flex justify-center py-6"><Loader2 className="h-5 w-5 animate-spin" style={{ color: "var(--color-subtle)" }} /></div>
        ) : sources.length === 0 ? (
          <p className="py-4 text-center text-sm" style={{ color: "var(--color-muted-foreground)" }}>{t("Base vacía. Cargá conocimiento o reindexá desde la plataforma.")}</p>
        ) : (
