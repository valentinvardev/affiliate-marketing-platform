...
function emojiCode(g: string): string {
  return [...g].map((c) => c.codePointAt(0)!.toString(16)).filter((c) => c !== "fe0f").join("-");
}
function loadEmoji(code: string): Promise<HTMLImageElement | null> {
  if (emojiCache.has(code)) return Promise.resolve(emojiCache.get(code)!);
  return new Promise((res) => {
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.onload = () => { emojiCache.set(code, img); res(img); };
    img.onerror = () => { emojiCache.set(code, null); res(null); };
    img.src = `https://cdn.jsdelivr.net/npm/emoji-datasource-apple@15.1.2/img/apple/64/${code}.png`;
  });
}
...
export function ImageEditor({ angleId, country, onClose }: { angleId: string; country: string; onClose: () => void }) {
  ...
  async function render(): Promise<Blob | null> {
    ...
    for (const l of layers) for (const line of l.text.split("\n")) for (const g of graphemes(line)) if (EMOJI_RE.test(g)) codes.add(emojiCode(g));
    const emo = new Map<string, HTMLImageElement | null>();
    await Promise.all([...codes].map(async (c) => emo.set(c, await loadEmoji(c))));
    ...
    for (const l of layers) {
      ...
      const sw = Math.max(1, fontPx * 0.05); // ~2px a resolución de export
            ...
            {bgUrl && layers.map((l) => (
              <div key={l.id} onPointerDown={(e) => onDown(e, l.id)}
                style={{
                  ...
                  WebkitTextStroke: "2px #000", paintOrder: "stroke", userSelect: "none",
